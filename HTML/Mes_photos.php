     ...
     if (isset($_POST['bouton'])) 
        {
            if ( isset($_FILES['fic']) )
            {
                ...
                $extensions_valides = array('jpg', 'jpeg', 'png'); /*formats supportés */
                ...
                if ($_FILES['fic']['size'] <= $taille_max) /* si l'image n'est pas trop grande */
                {
                    ...
                    if (in_array($extension_upload, $extensions_valides)) 
                    {
                        ...
                        if ($resultat) 
                        {
                            ...
                            $date = date('Y/m/d'); 
                            $img_lieu = "Où a été prise cette photo ?";
                            $img_theme = "Ajoutez un thème à cette photo";
                            $img_personnes = "Qui est sur cette photo ?";
                            $img_visibilite = "publique";
                            $img_jaime = 0;
                            $img_id_album = 0;

                            $insertimage = $bdd -> prepare("INSERT INTO  images(nom, taille, type, id_user, login_user, date_publication, lieu, theme, personnes_presentes, visibilite, jaime, id_album) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                            $insertimage -> execute(array($img_nom, $img_taille, $img_type, $img_user_id, $img_user_login, $date, $img_lieu, $img_theme, $img_personnes, $img_visibilite, $img_jaime, $img_id_album));
